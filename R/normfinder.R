...
#'
#' @param data A data frame with sample,gene and group identifier and expression values. Expression values can be raw Cq or on linear scale.
...
#' @param sample.cl numeric specifies the column of sample id
#' @param gene.cl numeric specifies the column of gene id
#' @param groups.cl numeric specifies the column of groups id
#' @param cq.cl numeric specifies the column of cq/expression values
#'
#' @import "dplyr"
#' @import "tidyr"
#' @export
normfinder<-function(data,Groups=TRUE,ctVal=TRUE,pStabLim=0.25,sample.cl=1,gene.cl=2,groups.cl=3,cq.cl=4){
  # Group & sample ID
  sample_group<-unique(data[,c(sample.cl,groups.cl)])
  
  tmp<-data.frame(sample=as.character(data[,sample.cl]),
                  gene=as.character(data[,gene.cl]),
                  cq=as.numeric(data[,cq.cl]))
  tmp<-tmp%>%
  ...
  MakeStabOne=function(da){
    ...
    for (gene in 1:ngenes){
      genevar0[gene]=
        sum((tmp[gene,]-geneavg[gene]-sampleavg+totalavg)^2)/
        ((ntotal-1)*(1-2/ngenes))
    }
